def strategy(state: dict) -> str:
    ...
    N_WIN_WEIGHT_SWITCH_T = 80
    RENDEZVOUS_TIMEOUT_T = 30
    ...
    defender_tag_radius: float = rule_config["blue_global"]["tagging_radius"]
    defender_sense_radius: float = rule_config["blue_global"]["sensing_radius"]
    stationary_sense_radius: float = rule_config["environment"]["blue_stationary_sensor_radius"]
    ...
    # ===== UPDATE POSITIONS =====
    enemy_team = agent_ctrl.enemy_team
    if "agent" in sensors:
        agent_sensor = agent_ctrl.sensor_data(state, "agent")
        for name, pos in agent_sensor["teammates"].items():
            agent_map.update_agent_position(team, name, pos, current_time)
        seen_enemy_names = set(agent_sensor["enemies"].keys())
        for name, pos in agent_sensor["enemies"].items():
    ...
    # ===== PROCESS FLAGS =====
    candidate_flags: List[int] = agent_ctrl.sensor_data(state, "candidate_flag")["candidate_flags"]
    real_flags: Set[int] = set(agent_ctrl.get_team("real_flags", []))
    fake_flags: Set[int] = set(agent_ctrl.get_team("fake_flags", []))
    discovered_flags = set()

    if "egocentric_flag" in sensors:
        detected_flags: List[int] = agent_ctrl.sensor_data(state, "egocentric_flag")["detected_flags"]
    ...
    agent_ctrl.set_team("real_flags", list(real_flags))
    agent_ctrl.set_team("fake_flags", list(fake_flags))
    ...
    def get_V_safe(defenders: List[int]) -> Set[int]:
        ...
        stationary_data = agent_ctrl.sensor_data(state, "stationary") or {"detections": []}
        for entry in stationary_data["detections"]:
            stationary_covered.update(entry.get("covered_nodes", []))
    ...
    if "agent" in sensors:
        attacker_positions.update(agent_ctrl.sensor_data(state, "agent")["teammates"])
    else:
    ...
    active_searcher = agent_ctrl.get_team("active_searcher", None)
    search_paused = bool(agent_ctrl.get_team("search_paused", False))

    if all_flags_checked:
        search_paused = False
        agent_ctrl.set_team("search_paused", False)
        ...
        agent_ctrl.set_team("active_searcher", active_searcher)
        agent_ctrl.set_team("search_target", None)
        search_paused = False
        agent_ctrl.set_team("search_paused", False)
    ...
    cached_capture_regions = agent_ctrl.get_team("_capture_region_by_flag", None)
    if (
        agent_ctrl.get_team("_capture_region_signature", None) != capture_region_signature
        ...
        agent_ctrl.set_team("_capture_region_by_flag", rebuilt_capture_regions)
        agent_ctrl.set_team("_capture_region_signature", capture_region_signature)
    ...
    cached_stationary_cover = agent_ctrl.get_team("_fallback_stationary_cover_nodes", None)
    if (
        agent_ctrl.get_team("_fallback_stationary_cover_signature", None) != stationary_cover_signature
        ...
        agent_ctrl.set_team("_fallback_stationary_cover_nodes", cached_stationary_cover)
        agent_ctrl.set_team("_fallback_stationary_cover_signature", stationary_cover_signature)
    ...
    _is_first_agent = not agent_ctrl.get_team(f"_dbg_tick_{current_time}", False)
    if _is_first_agent:
        agent_ctrl.set_team(f"_dbg_tick_{current_time}", True)
    ...
    def fallback_wait_move_from_strike() -> int:
        ...
        group_members = set(agent_ctrl.get_team("group_members", alive_attackers))
        my_wait = None
    ...
    def print_strike_fallback(context: str, strike_target: int, fallback_target: int):
        ...
        debug(
            ...
            f"team_phase={agent_ctrl.get_team('phase', None)}"
        )

    # ===== ESCAPE: if agent is inside no_enter_nodes, flee immediately =====
    phase = agent_ctrl.get_team("phase", "rendezvous")
    ...
    if current_pos in no_enter_nodes and not strike_active:
        ...
        if escape_candidates:
            safe_sub = get_safe_subgraph()
            def escape_score(n):
                dist = min(graph_distance(n, d) for d in defender_positions) if defender_positions else 0
                try:
                    comp = len(nx.node_connected_component(safe_sub, n)) if n in safe_sub else 1
            ...
            for n1 in agent_map.graph.neighbors(current_pos):
                for n2 in agent_map.graph.neighbors(n1):
                    if n2 not in no_enter_nodes:
                        dist = min(graph_distance(n2, d) for d in defender_positions) if defender_positions else 0
                        try:
                            comp = len(nx.node_connected_component(safe_sub, n2)) if n2 in safe_sub else 1
                        except nx.NodeNotFound:
                            comp = 1
                        score = (dist, comp)
                        if score > best_score:
        ...
        return f"phase={phase} target={state['action']}"
    ...
    if real_flags and phase in ("rendezvous", "search"):
        if phase == "rendezvous":
            meeting_point = agent_ctrl.get_team("meeting_point", None)
            ...
            if all_at_meeting:
                phase = "wait"
                agent_ctrl.set_team("phase", phase)
                if _is_first_agent:
    ...
    if phase == "rendezvous":
        ...
        last_recompute_tick = agent_ctrl.get_team("_meeting_recompute_tick", -1)
        if last_recompute_tick < current_time:
            agent_ctrl.set_team("_meeting_recompute_tick", current_time)
            ...
            if meeting_point is None or meeting_point not in rendezvous_candidates:
                ...
                agent_ctrl.set_team("meeting_point", meeting_point)
            else:
        ...
        if all_arrived or timed_out:
            ...
            agent_ctrl.set_team("phase", next_phase)
            phase = next_phase
            ...
            agent_ctrl.set_team("group_members", group_members)
            agent_ctrl.set_team("solo_members", solo_members)
            if _is_first_agent:
                if timed_out:
                    info(
    ...
    if phase == "search":
        if real_flags:
            ...
            agent_ctrl.set_team("phase", "wait")
            phase = "wait"
    ...
    if run_search_logic:
        ...
        if not unsensed_candidate_flags:
            ...
            search_target = agent_ctrl.get_team("search_target", None)
            chosen_target = None
            target = current_pos
            blocked_flags = []
            ...
            if chosen_target is None:
                ...
                if real_flags:
                    ...
                    agent_ctrl.set_team("search_paused", True)
                    phase = "wait"
                    info(
                        f"[t={current_time}][{agent_name}] SEARCHER -> wait "
                ...
                agent_ctrl.set_team("search_target", search_target)
                debug(
                    f"[t={current_time}][{agent_name}] SEARCH: explorer={active_searcher}, pos={current_pos}, "
    ...
    if phase == "wait":
        if not real_flags:
            agent_ctrl.set_team("phase", "search")
            state["action"] = current_pos
            return f"phase={phase} target={state['action']}"

        group_members = set(agent_ctrl.get_team("group_members", alive_attackers))

        # Explorer is only part of the group if co-located with other group members
        if active_searcher and not search_paused and not all_flags_checked:
            group_pos = {attacker_positions.get(n) for n in group_members if n != active_searcher}
            explorer_pos = attacker_positions.get(active_searcher)
            if explorer_pos is None or explorer_pos not in group_pos:
                group_members.discard(active_searcher)
        ...
        if is_group:
            ...
            agent_ctrl.set_team("wait_target", best_wait)

            # N_win from actual current position, not the theoretical wait node
            n_defenders = len(defender_positions)
            if best_wait_vt is not None and defender_positions:
                current_nwin = sum(1 for d in defender_positions
                                   if defender_advantage(best_wait_vt, d, current_pos) < 0)
            else:
                current_nwin = best_wait_nwin

            # Strike condition based on group size vs defenders
            n_loss = n_defenders - current_nwin
            current_k = N_WIN_WEIGHT_K_LATE if current_time >= N_WIN_WEIGHT_SWITCH_T else N_WIN_WEIGHT_K
            weighted_nloss = current_k * n_loss
            advantage_index = current_nwin - weighted_nloss
            # Group needs enough members to absorb losses and still have survivors to capture
            should_strike = current_nwin >= weighted_nloss and n_group > n_loss

            if _is_first_agent:
                debug(
                    f"[t={current_time}][{agent_name}] WAIT-GROUP: wait_node={best_wait}, strike_vt={best_wait_vt}, "
                    f"N_win={current_nwin}, K={current_k}, N_loss={n_loss}, adv_idx={advantage_index}, "
            ...
            if should_strike and best_wait_vt is not None and DEBUG_PHASE_ONLY != "wait":
                agent_ctrl.set_team("phase", "strike")
                agent_ctrl.set_team("strike_target", best_wait_vt)
                phase = "strike"
                if _is_first_agent:
    ...
    group_members_strike = set(agent_ctrl.get_team("group_members", alive_attackers))
    if phase == "strike" and agent_name in group_members_strike:
        strike_target = agent_ctrl.get_team("strike_target", None)
