...
def rollout_policy_combined_better(state, n_nodes, neighbours_cache, sp_cache, real_flags, min_actions_a=1,
                                   min_actions_d=1, n_closest_defenders=3):
...
def leaf_value_estimate_batch(att_pos: np.ndarray, def_pos: np.ndarray,
    ...
    value_cube = V_1v1[joint_states]  # (A, D, Nd, Na)
    ...
    for a in range(A):
        for d in range(D):
            ...
            values[a, d] = np.sum(mat[r_idx, c_idx]) / len(r_idx)

    return values


def filter_active_agents(attacker_dict, defender_dict, real_flag_nodes, candidate_flag_nodes, sp_cache, tag_radius=1,
                         capture_radius=2):
...
def get_or_solve_policy(graph, flags,n_defenders,capture_radius):
    ...
    if os.path.exists(fpath):
        ...
        game = TADGamePrimitives(graph=graph, goal_positions=flags,num_attackers = 1, num_defenders=n_defenders,capture_radius=2)
...
def run_regret_matching(attacker: dict, defender: dict, mamcts,
    ...
    if len(attacker) + len(defender) >=8:
        min_actions_d = 1
        n_simulations = 1000
...
def strategy(state: dict) -> str:
    global att_policy_1v1, def_policy_1v1, V_1v1, mamcts, last_time_attacker, real_flags_discovered
    ...
    # ===== FLAG DISCOVERY =====
    prev_flags = agent_ctrl.get_team("detected_flags", ([], 0))
    if flag_count > prev_flags[1]:
        agent_ctrl.set_team("detected_flags", (detected_flags, flag_count))

    best_flags = agent_ctrl.get_team("detected_flags", ([], 0))[0]
    real_flags_discovered = best_flags if best_flags else candidates
    # ===== INIT (t==1) =====
    if current_time == 1:
        # print('real_flag_discovered', real_flags_discovered)
        # print('candidate_flags', candidates)
        ap, dp, value = get_or_solve_policy(global_map_sensor, real_flags_discovered, 1, capture_radius)
        att_policy_1v1 = ap
        def_policy_1v1 = dp
        V_1v1 = value
        mamcts = MultiAgentMCTS(
            graph=global_map_sensor,
            flag_nodes=candidates,
            candidate_flag_nodes=candidates,
            k=0.5,
            tag_radius=int(opp_tagging_radius),
            capture_radius=int(capture_radius))

    if mamcts is not None:
        mamcts.flag_nodes = set(real_flags_discovered)

    # ===== DECISION LOGIC =====
    all_agents: Dict[str, int] = {**enemies, **teammates, agent_ctrl.name: current_pos}
    attackers, defenders = extract_attackers_and_defenders(all_agents)

    try:
        if agent_ctrl.get_team("att_action", None) is None or last_time_attacker != current_time:
            atk_action, _, _ = run_regret_matching(
                attacker=attackers,
                defender=defenders,
                mamcts=mamcts,
                n_simulations=2000,
                ...
                min_actions_a=2,
                min_actions_d=2)
            agent_ctrl.set_team("att_action", atk_action)
            last_time_attacker = current_time

        state["action"] = agent_ctrl.get_team("att_action", {}).get(agent_ctrl.name, current_pos)
