def strategy(state: dict) -> str:
    ...
    # possible targets contains a list of confirmed flag locations
    possible_targets : set[int] = agent_ctrl.get_team('possible_targets', None)
    ...
    for visible_node in visible_nodes.keys():
        # if the visible node is candidate flag and is detected as a flag add it to possible targets
        if (visible_node in candidates) and (visible_node in detected_flags):
            possible_targets.add(visible_node)
    # if we found a new flag, cache the results 
    ...
    remaining_candidates = set(candidates).difference(possible_targets)
    graph_nodes = list(u_graph.nodes)
    
    teammate_neighbours = set()
    for teammate_loc in teammates.values():
    ...
    grouping_weight_vector = GROUPING_WEIGHT * grouping_scores

    weighted_scores = goal_weight_vector + risk_weight_vector + grouping_weight_vector
    ...
    if previous_node is not None:
        weighted_scores[previous_node] += BACKTRACKING_PENALTY
