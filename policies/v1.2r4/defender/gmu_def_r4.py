...
def strategy(state):
    ...
    opp_capture_radius: float = rule_config["red_global"]["capture_radius"]
    ...
    if "custom_team" in state["sensor"]:
        for teammate_name, teammate_pos in agent_ctrl.sensor_data(state, "custom_team").items():
            agent_map.update_agent_position(team, teammate_name, teammate_pos, current_time)

    if "egocentric_agent" in state["sensor"]:
        _enemy = agent_ctrl.enemy_team
        for agent_name, node_id in agent_ctrl.sensor_data(state, "egocentric_agent")["enemies"].items():
    ...
    real_flags: List[int] = agent_ctrl.sensor_data(state, "flag")["real_flags"] if "flag" in sensors else []
    ...
    sensor_region = set()
    for entry in stationary["detections"]:
        sensor_region.update(entry.get("covered_nodes", []))
    ...
    if n_defenders != agent_ctrl.get_team("n_defenders_cached", -1):
        if not agent_ctrl.get_team("phase2_active", False):
            # Before Phase 2: redo Phase 1 precomputation with new defender count.
            agent_ctrl.set_team("positions_computed", False)
    ...
    visible_attackers: Dict[str, int] = dict(stationary["enemies"])
    ...
    # =========================================================
    if not agent_ctrl.get_team("positions_computed", False):
        if "flag" in sensors and agent_map.graph is not None and sensor_region:
            ...
            target_nodes = [
                ...
                if any(
                    agent_map.shortest_path_length(n, f) <= opp_capture_radius
                    for f in real_flags
                )
            ]
            ...
            # High-risk boundaries: within opp_capture_radius + 1 of any flag.
            # These require strict (<) coverage so that when an attacker actually
            # appears here, the rule flip from <= to < doesn't drop coverage.
            HIGH_RISK_RADIUS = opp_capture_radius + 1
            high_risk_boundary = {
                b for b in boundary_nodes
                if any(
                    agent_map.shortest_path_length(b, f) <= HIGH_RISK_RADIUS
                    for f in real_flags
                )
            }
            debug(f"[{agent_ctrl.name}] High-risk boundaries ({len(high_risk_boundary)}): "
                  f"{sorted(high_risk_boundary)}")
            ...
            if boundary_nodes and target_nodes:
                ...
                # High-risk boundaries demand strict beating; regular boundaries
                # allow a tie. Matches the Phase 2 cov_matrix rule so static
                # placement doesn't get immediately re-deployed.
                ...
                covers_boundary = {
                    p: {
                        ...
                        if (
                            DEPLOY_WEIGHT * min_deploy_dist[p] + p_reach_all[p][t]
                            < b_threat_boundary[b][t]
                            if b in high_risk_boundary else
                            DEPLOY_WEIGHT * min_deploy_dist[p] + p_reach_all[p][t]
                            <= b_threat_boundary[b][t]
                        )
                    }
                ...
                agent_ctrl.set_team("static_assignment", static_assignment)
                agent_ctrl.set_team("boundary_nodes", boundary_nodes)
                agent_ctrl.set_team("target_nodes", target_nodes)
                agent_ctrl.set_team("b_threat_boundary", b_threat_boundary)
                agent_ctrl.set_team("p_reach_all", p_reach_all)
                agent_ctrl.set_team("high_risk_boundary", high_risk_boundary)
                agent_ctrl.set_team("n_defenders_cached", n_defenders)
                agent_ctrl.set_team("positions_computed", True)
    ...
    # =========================================================
    static_assignment: Dict[str, int] = agent_ctrl.get_team("static_assignment", {})
    ...
    if not agent_ctrl.get_team("phase2_active", False) and (all_deployed or bool(visible_attackers)):
        agent_ctrl.set_team("phase2_active", True)
    ...
    # =========================================================
    if agent_ctrl.get_team("phase2_active", False) and agent_ctrl.get_team("positions_computed", False):

        if agent_ctrl.get_team("dynamic_time") != current_time:
            import itertools as _it
            import numpy as np

            graph = agent_map.graph
            boundary_nodes: List[int] = agent_ctrl.get_team("boundary_nodes", [])
            target_nodes: List[int] = agent_ctrl.get_team("target_nodes", [])
            b_threat_boundary: Dict[int, Dict[int, int]] = agent_ctrl.get_team("b_threat_boundary", {})
            p_reach_all: Dict[int, Dict[int, int]] = agent_ctrl.get_team("p_reach_all", {})
            high_risk_boundary: set = agent_ctrl.get_team("high_risk_boundary", set())
                ...
                for j, (b, t) in enumerate(all_threat_pairs):
                    if b in attacker_positions:
                        ...
                        if p_reach_p.get(t, float("inf")) < threat_dist[b][t]:
                            cov_matrix[ci, j] = 1
                    elif b in high_risk_boundary:
                        # Strict: near-flag boundary needs a 1-step buffer so
                        # coverage survives an attacker appearing here (the rule
                        # flips from <= to < the moment the attacker materialises).
                        if p_reach_p.get(t, float("inf")) < threat_dist[b][t]:
                            cov_matrix[ci, j] = 1
                    else:
                        # Non-strict: a tie (advantage = 0) is acceptable for
                        # far hypothetical boundary attackers.
                        if p_reach_p.get(t, float("inf")) <= threat_dist[b][t]:
                            cov_matrix[ci, j] = 1
            ...
            # Six-tier lexicographic score (all maximised). Priority high → low:
            #   1. atk_margins            — min margin over visible-attacker pairs (strict <)
            #   2. atk_sum_margins        — sum of margins over visible-attacker pairs
            #   3. high_risk_bnd_margins  — min margin over near-flag boundaries (strict <).
            #                               Sits above the conservative/aggressive swap so
            #                               aggressive tag-chasing can never sacrifice it.
            #   4. bnd_margins_regular    — min margin over far boundaries (non-strict <=)
            #   5. bnd_covered_count      — # far-boundary pairs with margin >= 0
            #   6. tag_scores             — proximity to visible attackers
            # Aggressive mode swaps tag_scores above bnd_margins_regular and bnd_covered_count,
            # but the high-risk tier (3) stays put.
            if atk_pair_indices:
            ...
            # Split boundary pairs into high-risk (near flag, strict) and regular (far, loose).
            atk_pair_set = set(atk_pair_indices)
            high_risk_indices = [
                j for j, pair in enumerate(all_threat_pairs)
                if pair[0] in high_risk_boundary and j not in atk_pair_set
            ]
            regular_bnd_indices = [
                j for j, pair in enumerate(all_threat_pairs)
                if pair[0] not in high_risk_boundary and j not in atk_pair_set
            ]
            if high_risk_indices:
                high_risk_bnd_margins = margins_all[:, high_risk_indices].min(axis=1)
            else:
                high_risk_bnd_margins = np.zeros(len(all_combos), dtype=np.int16)
            if regular_bnd_indices:
                bnd_margins_regular = margins_all[:, regular_bnd_indices].min(axis=1)
                bnd_covered_count = (margins_all[:, regular_bnd_indices] >= 0).sum(axis=1)
            else:
                bnd_margins_regular = np.zeros(len(all_combos), dtype=np.int16)
                bnd_covered_count = np.zeros(len(all_combos), dtype=np.int16)
            ...
            # Conservative 6-key sort: regular-boundary slack outranks tagging.
            best_idx_conservative = int(np.lexsort((
                tag_scores,
                bnd_covered_count,
                bnd_margins_regular,
                high_risk_bnd_margins,
                atk_sum_margins,
                atk_margins,
            ))[-1])

            # Aggressive mode: when attacker coverage is in hand, promote tagging
            # above the regular boundary tiers. high_risk_bnd_margins stays put so
            # near-flag coverage is never sacrificed for tag proximity.
            if atk_pair_indices and atk_margins[best_idx_conservative] >= 0:
                best_idx = int(np.lexsort((
                    bnd_covered_count,
                    bnd_margins_regular,
                    tag_scores,
                    high_risk_bnd_margins,
            ...
            hr_covered = sum(1 for j in high_risk_indices if best_margins[j] >= 0)
            bnd_covered = sum(1 for j in regular_bnd_indices if best_margins[j] >= 0)
            debug(f"[{agent_ctrl.name}] Phase2 t={current_time} | "
                  f"{len(all_combos)} combos | "
                  f"atk_worst={int(atk_margins[best_idx])} "
                  f"atk_total={int(atk_sum_margins[best_idx])} "
                  f"hr_worst={int(high_risk_bnd_margins[best_idx])} "
                  f"bnd_worst={int(bnd_margins_regular[best_idx])} "
                  f"bnd_covered_score={int(bnd_covered_count[best_idx])} "
                  f"tag_score={float(tag_scores[best_idx]):.1f} "
                  f"mode={_tagging_mode} | "
                  f"ATK:{atk_covered}/{len(atk_pair_indices)} "
                  f"HR:{hr_covered}/{len(high_risk_indices)} "
                  f"BND:{bnd_covered}/{len(regular_bnd_indices)} | "
                  f"attackers={dict(sorted(visible_attackers.items()))}")

            agent_ctrl.set_team("dynamic_assignment", dynamic_assignment)
            agent_ctrl.set_team("dynamic_time", current_time)

        # Move to the assigned next position (already 1 step away by construction)
        dynamic_assignment: Dict[str, int] = agent_ctrl.get_team("dynamic_assignment", {})
    ...
    # =========================================================
    elif agent_ctrl.get_team("positions_computed", False):
