...
def strategy(state):
    ...
    agent_ctrl.set_team("last_update", current_time)
    agent_ctrl.update_team(total_tags=agent_ctrl.get_team("total_tags", 0), formation="defensive")
    ...
    if not team_cache.has('Attacker_Initial_Entry'):
       agent_ctrl.set_team('Attacker_Initial_Entry',{})
    ...
    # Getting the stationary
    stationary_data = agent_ctrl.sensor_data(state, "stationary") or {}
    attacker_nodes_dic = dict(stationary_data.get("enemies", {}))
    
    # Getting both the attacker nodes from stationary nodes and agent sensor obsereved enemy nodes...
    attacker_nodes = list({x for x in list(attacker_nodes_dic.values()) if x is not None} | {x for x in list(nearby_enemies.values()) if x is not None})
    ...
    # Take difference between the Vprotect and Vtag for each agent...
    Vprotect_Vban_Area_dic = {i:None for i in Vcand}
    for i in Vcand:
        Vprotect_Vban_Area_dic[i] = Vprotect_dic[i] - Vbanned[i]

    # Dictionary to maintain the attacker agent locaitons on the Vgreen_Vprotect_Area_dic region...
    ...
    # Dictionary to maintain the attacker agent location on the Vcand_best with Vgreen_Vban_Area_dic region...
    Vgreen_Vban_Area_dic = {i:None for i in Vcand}
    for i in Vcand:
        Vgreen_Vban_Area_dic[i] = Vgreen_dic[i] - Vbanned[i]

    # Sorted List of attackers in Vcand_best's Vgreen and Vban...
    Vgreen_Vban_Area_Attacker_log = {i:None for i in Vcand}
    for i in Vcand:
        Vgreen_Vban_Area_Attacker_log[i] = [x for x in attacker_nodes if x in  Vgreen_Vban_Area_dic[i]]

    # Sorted List of attackers in Vcand_best's Vgreen and Vban...
    priority_ranked_Vattackers, _ = _sort_nodes_by_min_hops_to_targets(global_map_sensor,set(Vgreen_Vban_Area_Attacker_log[agent_ctrl.get_team("Vcand_best")]),Vbanned_boundary)
    ...
    # print(f'Priority Ranked Vattacker: {priority_ranked_Vattackers}, Nodes list: {Vgreen_Vban_Area_dic[agent_ctrl.get_team("Vcand_best")]}')
    # ===== DECISION LOGIC ====
    path_to_min_node = []
    global_assigned_defender = []
    global_unassigned_defenders = []

    # Gettng the list of attacker node that are in 'Vcand_best' zone...
    if len(priority_ranked_Vattackers) != 0:
       assigned_defender_r0 = []
       # Getting each attacker in priority ranked Vattacker...
       for attacker in priority_ranked_Vattackers:
           # Getting agent defender for given attacker with minimum hops...
           best_defender,_ = _min_hop_target(global_map_sensor,attacker,[x for x in teammate_nodes if x not in set(assigned_defender_r0)])
           ...
           assigned_defender_r0.append(best_defender)
       # Getting the list of unassinged defenders...
       unassinged_defenders_r0 = [x for x in teammate_nodes if x not in set(assigned_defender_r0)]
       # Getting the list of flag allocated defender list...
       allocated_defenders_r0 = []
       # Getting the unassinged list of defenders alloted to flags...
       for flag in Vflags:
           # Getting agent defender for given attacker with minimum hops...
           best_defender,_ = _min_hop_target(global_map_sensor,flag,[x for x in unassinged_defenders_r0 if x not in set(allocated_defenders_r0)])
           if current_pos == best_defender: 
              min_node = attacker
              path_to_min_node = nx.shortest_path(global_map_sensor, source=current_pos, target=attacker)
           # Adding the best assigned defender to assinged list... 
           allocated_defenders_r0.append(best_defender)
       # Updatng the global list values...
       global_assigned_defender.extend(assigned_defender_r0)
       global_assigned_defender.extend(allocated_defenders_r0)
       global_unassigned_defenders.extend(unassinged_defenders_r0)


    
    # Getting the list of attacker nodes that are defender sensing zone...
    if (len(high_ranked_Vattacker) != 0) and (len(global_unassigned_defenders) != 0):
